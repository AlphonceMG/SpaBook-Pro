...
class MpesaController {
    ...
    try {
        ...
        const password = Buffer.from(`${bs_short_code}${passkey}${timestamp}`).toString('base64');
        const transcation_type = "CustomerPayBillOnline";
        const amount = "1"; // You can enter any amount
        const partyA = "254793004830"; // Should follow the format: 2547xxxxxxxx
        const partyB = process.env.lipa_na_mpesa_shortcode;
        const phoneNumber = "254793004830"; // Should follow the format: 2547xxxxxxxx
     ...
     lipaNaMpesaOnlineCallback(req, res) {
        try {
            ...
            const mpesaReceiptNumber = req.body.Body.stkCallback['CallbackMetadata'].Item.find(item => item.Name === 'MpesaReceiptNumber').Value;
