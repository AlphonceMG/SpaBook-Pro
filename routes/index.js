...
router.get('/mpesa/payment-status', mpesa.getOAuthToken, async (req, res) => {
    try {
        ...
        const apiEndpoint = 'https://sandbox.safaricom.co.ke/mpesa/transactionstatus/v1/query';
        ...
        const requestBody = {
           "Initiator":"eddas-spa",
            ...
            "ResultURL":"https://e455-102-212-236-174.ngrok-free.app/transactionstatus/result",
            "QueueTimeOutURL":"https://e455-102-212-236-174.ngrok-free.app/timeout",
            "Remarks":"OK",
            "Occasion":"OK",
        };
        ...
        if (response.status === 200) {
            ...
            if (apiResponse.ResponseCode === '0') {
                ...
                return res.render('payment-success', { mpesaReceiptNumber: apiResponse.CheckoutRequestID });
            } else {
                // Payment was not successful, render the payment error page with the error message
                return res.render('payment-error', { errorMessage: apiResponse.ResponseDescription });
            }
        } else {
